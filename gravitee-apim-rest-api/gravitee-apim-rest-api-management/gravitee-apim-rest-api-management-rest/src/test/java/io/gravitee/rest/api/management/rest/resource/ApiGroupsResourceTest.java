...
public class ApiGroupsResourceTest extends AbstractResourceTest {
    ...
    public void shouldReturn404IfNotFound() {
        when(permissionService.hasPermission(RolePermission.API_MEMBER, API_ID, RolePermissionAction.READ)).thenReturn(true);

        when(apiService.getGroupsWithMembers(API_ID)).thenThrow(new ApiNotFoundException(API_ID));
    ...
    public void shouldReturn500IfTechnicalException() {
        ...
        when(apiService.getGroupsWithMembers(API_ID)).thenThrow(new TechnicalManagementException());
    ...
    @Test
    public void shouldReturnEmptyListIfNotGranted() {
        Response response = envTarget(API_ID).path("groups").request().get();
    ...
    public void shouldReturnGroupsIfGranted() {
        ...
        when(apiService.getGroupsWithMembers(API_ID)).thenReturn(Map.of(UuidString.generateRandom(), List.of(new GroupMemberEntity())));
